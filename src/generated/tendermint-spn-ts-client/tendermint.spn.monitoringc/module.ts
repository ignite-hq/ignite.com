...
class Module extends Api<any> {
	...
	async sendMsgCreateClient({ value, fee, memo }: sendMsgCreateClientParams): Promise<DeliverTxResponse> {
		...
		if (!this._addr) {
            throw new Error('TxClient:sendMsgCreateClient: Unable to sign Tx. Address is not present.')
        }
		try {
	...
	msgCreateClient({ value }: msgCreateClientParams): EncodeObject {
		try {
			return { typeUrl: "/tendermint.spn.monitoringc.MsgCreateClient", value: MsgCreateClient.fromPartial( value ) }  
		} catch (e: any) {
